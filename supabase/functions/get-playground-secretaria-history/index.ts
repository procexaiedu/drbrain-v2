...
// Interface para a estrutura da mensagem como vem do DB
...
console.log('Function get-playground-secretaria-history loaded');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Autenticação (opcional, mas recomendado se chamada diretamente do frontend)
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select('id, session_id, message') // Se tiver timestamp, adicione aqui
      .eq('session_id', medico_id) // medico_id corresponde a session_id
      .order('id', { ascending: true }); // Ordenar por ID para manter a ordem de inserção

    if (error) {
      console.error('Erro ao buscar histórico da Secretária IA (Playground):', error);
      return new Response(JSON.stringify({ error: 'Falha ao buscar histórico', details: error.message }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 500,
      });
    }
    ...
    const transformedMessages = data.map((dbRow: HistoryMessageFromDB) => {
      // A coluna 'message' é um JSONB, então dbRow.message já é um objeto
      const messageContent = dbRow.message;
      return {
        id: `pg_sia_db_${dbRow.id}`,
        text: extractCleanContent(messageContent.content),
        sender: messageContent.type === 'human' ? 'user' : 'agent',
        // Idealmente, sua tabela de memória teria um timestamp por mensagem.
        // Se não, podemos usar o timestamp da consulta ou um fallback.
        timestamp: new Date().toISOString(), // Fallback, idealmente viria do DB
      };
    });
    ...
    console.error('Erro na Edge Function get-playground-secretaria-history:', error);
