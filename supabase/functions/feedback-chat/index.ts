...
console.log('Function feedback-chat loaded');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Validar se medico_id corresponde ao usuário autenticado
    if (user.id !== medico_id) {
      return new Response(JSON.stringify({ error: 'medico_id does not match authenticated user' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 403,
      });
    }
    ...
    // Chamar webhook do N8N para feedback - enviando o payload completo
    ...
    console.error('Unhandled error in feedback-chat:', error);
