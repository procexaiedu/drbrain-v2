...
const WhatsappConnectionCard: React.FC = () => {
  ...
  const { data: connectionStatus, isLoading, error } = useQuery<ConnectionStatus>({
    ...
    refetchInterval: (query) => {
      ...
      return data?.status === 'open' || data?.status === 'connected' ? false : 5000;
    },
  });
  ...
  const isPending = connectionStatus?.status === 'pending' || connectionStatus?.status === 'pairing' || connectionStatus?.status === 'connecting';
